# Topics Backlog — Charter Select Hub Library

The single tracker for this engine. The `deai-editor` agent updates the Status column as papers move through the pipeline. Notion is intentionally not wired up; this file is the tracker until there are 10+ assets in flight.

**Status values:** Queued, Drafted, Drafted + Sliced.
**Verify:** mechanics must be confirmed by `insurance-researcher` before writing.

## Pillar 1: Governance & Legal Shield
...
| Sexual Abuse & Molestation as a standalone line | sam-coverage | Drafted + Sliced | yes (verified w/ sources) |
...
| Extra Expense Coverage | extra-expense-coverage | Drafted + Sliced | no |
| Smart Risk Mitigation (IoT sensors) | smart-risk-mitigation | Drafted + Sliced | no |
| Facility Use Agreements (Additional Insured) | facility-use-agreements | Drafted + Sliced | no |
| Disaster Blueprint (claims checklist) | disaster-blueprint | Drafted + Sliced | no |
| Environmental / Pollution (mold, lead, old buildings) | environmental-pollution | Drafted + Sliced | no |
...
| Student Accident Insurance | student-accident-insurance | Drafted + Sliced | no |
| Program Value-Adds (carrier perks audit) | program-value-adds | Drafted + Sliced | no |
| Workers' Comp Experience-Mod & Class Codes | workers-comp-experience-mod | Drafted + Sliced | no |
...
| Cost Controls (Reference-Based Pricing) | reference-based-pricing | Drafted + Sliced | yes (verified w/ sources) |
| Advanced Employee Care (DPC, transparent Rx) | advanced-employee-care | Drafted + Sliced | no |
| Open Enrollment Modernization | open-enrollment-modernization | Drafted + Sliced | no |
| Stop-Loss Contract Terms & Lasering | stop-loss-terms-lasering | Drafted + Sliced | yes (verified w/ sources) |
...
- Total topics: 28
- Drafted + Sliced: 28
- Drafted (hub written, awaiting review then slicing): 0
- Queued: 0
...
Founder review pass (in progress): four Governance/Continuity papers were repositioned on the founder's direction to a consistent editorial line, do not reflexively sell a standalone specialty policy, check what the package already covers and run a needs/risk-tolerance analysis. employed-legal-counsel (employed-lawyer coverage belongs in the management liability definition, standalone is the generalist over-sell), management-liability-placement (School Leaders coverage with the package carrier under a following-form program umbrella, scattered standalone tower is the over-sell), board-side-a-protection (individual-director protection built into the School Leaders program, bolted-on external Side-A DIC is the over-sell), and active-assailant-coverage (a trigger-vs-fear and buy-vs-self-fund decision, not a default buy). Hubs and spokes both updated. A second round extended the same lens to three more: crime-vs-cyber (reframed to what-are-you-protecting-against and the need for both, keeping the social-engineering seam), environmental-pollution (a needs analysis: authorizer/state/lease requirement vs real exposure, then package endorsement vs standalone), and sam-coverage (adequate SAM built into a real school program at meaningful limits; monoline SAM is almost never the answer and, if it is all you can get, a red flag about the placement). Seven Governance/Continuity papers now share the through-line: do not reflexively sell a standalone specialty policy, understand the exposure, check what the package already does, then decide placement and risk tolerance on purpose.
